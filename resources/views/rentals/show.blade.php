        ...
        <div class="row gy-4">
                    ...
                    @if ($rental->images && count($rental->images) > 0)
                        <div id="rentalCarousel" class="carousel slide" data-bs-ride="carousel">
                            <div class="carousel-inner">
                                @foreach ($rental->images as $index => $image)
                                    <div class="carousel-item {{ $index === 0 ? 'active' : '' }}">
                                        <img src="{{ asset('storage/' . $image) }}" class="d-block w-100"
                                            alt="{{ $rental->name }}" loading="lazy"
                        ...
                        @if (count($rental->images) > 1)
                            <div class="mt-3">
                                <div class="row g-2">
                                    @foreach ($rental->images as $index => $image)
                                        <div class="col-3">
                                            <img src="{{ asset('storage/' . $image) }}" class="img-thumbnail cursor-pointer"
                                                alt="Thumbnail {{ $index + 1 }}" onclick="goToSlide({{ $index }})"
            ...
            <div class="col-12 col-lg-6">
                ...
                <div class="d-flex align-items-center mb-3 flex-wrap gap-2">
                    ...
                    @endif
                    @if ($rental->images && count($rental->images) > 1)
                        <span class="badge bg-info">{{ count($rental->images) }} foto</span>
                    @endif
                </div>
